...
class EmailHtmlBackend(backends.BaseBackend):
    spam_sensitivity = 2

    def can_send(self, user, notice_type):
        can_send = super(EmailHtmlBackend, self).can_send(user, notice_type)
        if can_send and user.email:
            return True
        return False
    ...
    def deliver(self, recipient, sender, notice_type, extra_context):
        ...
        # send_mail(subject, body, settings.DEFAULT_FROM_EMAIL, [recipient.email])
        msg = EmailMultiAlternatives(subject, body, settings.DEFAULT_FROM_EMAIL, [recipient.email])
